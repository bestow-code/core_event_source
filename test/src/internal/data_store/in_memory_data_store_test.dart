  ...
  given('data store - empty', () {
    ...
    then('check state', () async {
      ...
      final rootEntrySnapshot =
          (await dataStore.entrySnapshotsStream.take(1).toList()).first.single;
      check(rootEntrySnapshot.entry.ref).equals(EntryRef.root);
    });
    when2('add entry', () async => dataStore.appendHeadEntry(entry1), then: () {
    ...
    when2('set mainEntryRef', () async => dataStore.setMainEntryRef(entry1.ref),
        then: () {
      then('check state and entry collection stream', () async {
