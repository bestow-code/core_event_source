...
main() {
  ...
  late DataAdapterFirestore<Event> adapter;
  late Entry<Event> initialEntryIfEmpty;
  ...
  setUp(() {
    ...
    adapter = DataAdapterFirestore(entryStore);

    initialEntryIfEmpty =
        Entry<Event>.initial(createdAt: dateTimeFactory.create());
  });
  group('initialize', () {
    ...
    test('happy path - data exists', () async {
      ...
      await previousAdapter.initialize(initialEntryIfEmpty);
      await adapter.initialize(initialEntryIfEmpty);
      final initialEntry = await adapter.rootEntry;
      expect(initialEntry.ref.value, EntryRef.root.value);
      expect(initialEntry.refs.isEmpty, true);
  ...
  test('entryQuerySnapshots', () async {
    ...
    expect((await adapter.entrySnapshotsStream.first).toList(),
        [EntrySnapshot(initialEntryIfEmpty, isPending: false)]);
  });
}
